...
def oversample(wave, flux, unc, num, scale = 1.0):
    '''
    A function that oversamples the given fluxes by adding noise
    ...
    num : integer
                    Determines the number of oversampled data
    ...
    otemplate : pandas.Dataframe
                    A dataframe whose rows are the oversampled data and whose columns are the flux and unc values
    
    all_oflux = []
    all_ounc = []
    
    for i in range(num):
        oflux, ounc = addNoise(wave, flux, unc, scale=1.0)
        all_oflux.append(oflux)
        all_ounc.append(ounc)
        
    all_oflux = pd.concat(all_oflux)
    all_ounc = pd.concat(all_ounc)
    
    otemplate = pd.concat([all_oflux, all_ounc], axis = 1)
